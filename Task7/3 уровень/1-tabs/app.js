...
navLinks.forEach(navLink => navLink.addEventListener('click', clickHandler,));
...
function changeActiveClass(event) {
    ...
    // }else if(event.classList.contains("active")!=true) {
...
const innerText = (event) =>  {
    ...
    if(event.target.textContent === 'Link 1'){
        AllText.innerHTML = texts.text1
    }else if(event.target.textContent === 'Link 2'){
        AllText.innerHTML = texts.text2
    }else if(event.target.textContent === 'Link 3'){
        AllText.innerHTML = texts.text3
    }
}
/**
...
function changeText(event) {
    ...
    //Пока не понял как тут обьект перебрать
